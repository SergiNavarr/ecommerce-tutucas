    ...
    return (
        <Link href={`/product/${product.attributes.slug}`}
            ...
            <div className="absolute flex items-center justify-between gap-3 px-2 z-[1] top-4">
                <p
                    className="px-2 py-1 text-xs text-white bg-black rounded-full dark:bg-white dark:text-black w-fit">{product.attributes.productName}</p>
                <p
                    className="px-2 py-1 text-xs text-white bg-purple-900 rounded-full dark:bg-white dark:text-black w-fit">${ formatPrice(product.attributes.price) }</p>
            </div>
            <Carousel
                opts={{
                    align: "start",
